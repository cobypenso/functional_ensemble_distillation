...
def run_epoch(args, cfg, epoch_idx, model, dataloaders, logger,
    ...
    for batch_idx, (images, labels) in enumerate(dataloader, start=1):
        ...
        if is_train:
            images = images.repeat(args.be_batch_repetitions, 1, 1, 1)
            labels = labels.repeat(args.be_batch_repetitions)

        else:
            images = images.repeat(cfg.MODEL.BATCH_ENSEMBLE.ENSEMBLE_SIZE, 1, 1, 1)
            labels = labels.repeat(cfg.MODEL.BATCH_ENSEMBLE.ENSEMBLE_SIZE)
        ...
        if not is_train:
            outputs["confidences"] = torch.stack(
                torch.split(
                    outputs["confidences"],
                    outputs["confidences"].size(0) // cfg.MODEL.BATCH_ENSEMBLE.ENSEMBLE_SIZE
                ), dim=1
            ).mean(1)
            outputs["labels"] = torch.split(
                outputs["labels"],
                outputs["labels"].size(0) // cfg.MODEL.BATCH_ENSEMBLE.ENSEMBLE_SIZE
            )[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--be-batch-repetitions", default=4, type=int,
