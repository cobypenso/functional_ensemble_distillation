...
def run_epoch(args, cfg, epoch_idx, model, dataloaders, logger,
    ...
    loss_bce_meter = {"sum": 0.0, "avg": 0.0, "cnt": 0,}
    loss_gp_meter = {"sum": 0.0, "avg": 0.0, "cnt": 0,}
    ...
    for batch_idx, (images, labels) in enumerate(dataloader, start=1):
        ...
        _classifier = model.classifier if cfg.NUM_GPUS == 1 else model.module.classifier

        # compute loss        
        outputs = model(images, labels)
        loss_bce = F.binary_cross_entropy(
            input     = outputs["confidences"],
            target    = F.one_hot(outputs["labels"], _classifier.num_classes).float(),
            reduction = "mean",
        )
        
        # compute gradient penalty
        grad = torch.autograd.grad(
            outputs["confidences"], outputs["images"],
            grad_outputs=torch.ones_like(outputs["confidences"]),
            create_graph=True,
        )[0].flatten(start_dim=1)
        grad_norm = grad.norm(2, dim=1)
        loss_gp = ((grad_norm - 1.0) ** 2).mean()
        loss = loss_bce + args.duq_grad_penalty * loss_gp
        ...
        # update centroids of DUQ
        _classifier.update_centroids(outputs["features"], outputs["labels"])
        ...
        update_meter(loss_bce_meter, loss_bce, outputs["labels"].size(0))
        update_meter(loss_gp_meter, loss_gp, outputs["labels"].size(0))
        ...
        if is_train and (batch_idx in _logging_batches):
            ...
            log_str += "loss: {:.4e} ({:.4e}), loss_bce: {:.4e} ({:.4e}), loss_gp: {:.4e} ({:.4e}), acc1: {:.4f} ({:.4f}), acc5: {:.4f} ({:.4f}), lr: {:.8f}, max_mem: {:.0f}M".format(
                loss, loss_meter["avg"],
                loss_bce, loss_bce_meter["avg"],
                loss_gp, loss_gp_meter["avg"],
                acc1, acc1_meter["avg"],
                acc5, acc5_meter["avg"],
    ...
    log_dict = {
        ...
        "trn/loss_bce" if is_train else "val/loss_bce": loss_bce_meter["avg"],
        "trn/loss_gp" if is_train else "val/loss_gp": loss_gp_meter["avg"],
    ...
    log_str += "loss: {:.4e}, loss_bce: {:.4e}, loss_gp: {:.4e}, acc1: {:.4f}, acc5: {:.4f}".format(
        log_dict["trn/loss" if is_train else "val/loss"],
        log_dict["trn/loss_bce" if is_train else "val/loss_bce"],
        log_dict["trn/loss_gp" if is_train else "val/loss_gp"],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--duq-grad-penalty", default=0.0, type=float,
                        help="weights for two-sided gradient penalty")
