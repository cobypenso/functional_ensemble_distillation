...
class SyntheticRegressionData(torch.utils.data.Dataset):
    ...
    def sample_new_data(self):
        ...
        if self.train:
            x_1 = np.random.normal(-4, 0.4, [int(np.ceil(self.n_samples / 3)), 1])
            x_2 = np.random.normal(0, 0.9, [int(np.ceil(self.n_samples / 3)), 1])
            x_3 = np.random.normal(4, 0.4, [self.n_samples - 2 * int(np.ceil(self.n_samples / 3)), 1])
            all_x = np.row_stack([x_1, x_2, x_3])

        else:
            all_x = np.linspace(-5, 5, self.n_samples)[:, np.newaxis]

        all_y = 7 * np.sin(all_x) + 3 * np.abs(np.cos(all_x / 2)) * np.random.normal(size=[self.n_samples, 1])
