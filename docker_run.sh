...
if [[ "${CONTAINER_ID}" ]]; then
    docker run --shm-size=2g --name fed_container --gpus all -it --rm -d -v `pwd`:/scratch --user $(id -u):$(id -g) \
        --workdir=/scratch -p 8888:8888 -p 1234:1234 -e HOME=/scratch $IMAGE $@ bash
else
